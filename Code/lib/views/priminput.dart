...
import 'package:graphalgorithms/views/showprim.dart';

class PrimPage extends StatefulWidget {
  PrimPage({Key key, this.title}) : super(key: key);
  final String title;
...
class _PrimPageState extends State<PrimPage> { // page that takes number of vertexes for prim with error handling
  @override
      ...
      onPressed: () async {
        if (!vertexController.text.isNotEmpty) {
          ...
          Navigator.push(
            ...
            MaterialPageRoute(
                builder: (context) =>
                    ShowPrim(vertex: int.tryParse(vertexController.text))),
          );
        }
      },
      child: Text(
